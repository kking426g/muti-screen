...
var Game_f = require('./flip_game');
var Game_p = require('./paper_game');
var Game_m = require('./mono_game');
var Game_s = require('./seven_game');
...
app.get('/table', function (req, res) {
    res.sendfile(__dirname + '/new-table.htm');
});
...
app.use('/clientJS', express.static(__dirname + '/clientJS'));//20160922 Joanne
...
//電視為一局
	socket.on('tv',function(){
			...
			var sql= "INSERT INTO Inning(inning_guid) VALUES('"+key+"')";//開局
			...
			var sql_d= "UPDATE Inning SET t_time ='"+ date +"' WHERE inning_guid ='"+key+"'";

			console.log(sql_d);
			db.query(sql_d, function(err, result) {
				if(err){
					console.log(err);
					return ;
					}
			...
			db.query('SELECT g_name,game_guid FROM ' + 'Game', function(err, results, fields){
				...
				for(var i=0;i<all_game.length;i++){
					all_game_guid.push(all_game[i].game_guid);
				}
	...
	var key;
		...
		//刪掉offline的人
		var d = "DELETE from Inning_user where online ='"+ 0 +"'";
		console.log(d);
		db.query(d, function(err, result) {
				if(err){
					console.log(err);
					return ;
				}
		...
		//找資料庫有沒有這個人，不然就新增
		var data = "SELECT * from Inning_user where user_account ='"+ p_name +"' AND user_pic = '"+ pic +"' AND online='1' ";
				...
				if(result[0] == undefined && p_name!=''){//ining_user沒有這個人
					console.log('現在-----> result[0]== undefined, 這個人沒有在inning_user資料表裡面');
				//判斷user裡面有沒有這個人
				var data_u = "SELECT * from User where account ='"+ p_name +"'";

				db.query(data_u, function(err, result) {
					if(err){
						console.log(err);
						return ;
					}

					if(result[0]== undefined && p_name!=''){//如果沒有

						console.log('現在-----> result[0]== undefined, 這個人沒有在user資料表裡面');
						//新增帳號 加入inning_user
					key=uuid.v4();
					console.log(key);
					//var p_name = pick_name(key);

					//加入此人到user
					var sql= "INSERT INTO User(user_guid,account,pic) VALUES('"+key+"','"+ p_name+"','"+ pic+"')";
					console.log(sql);
					db.query(sql, function(err, result) {
							if(err){
								console.log(err);
					...
					db.query(sql_p_sum, function(err, result) {
						...
						if(sum ==0){
							...
							//把此人選的遊戲guid放入此局
							var sql_u = "UPDATE Inning SET g_gref ='"+ player_g +"' WHERE inning_guid ='"+player_i+"'";
							console.log(sql_u);
							db.query(sql_u, function(err, result) {
								if(err){
									console.log(err);
									return ;
								}
							...
							var key_iu=uuid.v4();
							console.log(key_iu);
							var sql_iu = "INSERT INTO Inning_user(iu_guid,inning_gref,game_guid, user_gref,user_account,user_pic) VALUES('"+key_iu+"','"+player_i+"','"+ player_g +"','"+ key +"','"+ p_name +"','"+ pic +"')";
							console.log(sql_iu);
							...
							//all_socket.push(player_i);//把這個電視加入所有socket裏面

							console.log('電視連線：'+all_socket[ player_i ]);
							all_socket[ player_i ].emit('tv_newplayer', p_name);
						}

						else if((sum+1) >= g_data[0].g_p_less && (sum+1) < g_data[0].g_p_more){//人數可以開始
							socket.emit('client_change',p_name, key, player_i, g_data, sum);
							all_socket[ player_i ].emit('tv_newplayer', p_name);
							console.log('問他要不要開始');
							socket.emit('decide');//問他要不要開始
							//改變那個人的畫面（問他要不要開始），不要開始就不動作
							//要開始就讓他開始
						}

						//else if((sum+1) < g_data[0].g_p_less)


						else if((sum+1) == g_data[0].g_p_more){

							game_start(key, p_name, pic);//開始遊戲的function
							socket.emit('client_change',p_name, key, player_i, g_data, sum);
							all_socket[ player_i ].emit('tv_newplayer', p_name);
					...
					console.log("所有的all_socket："+ all_socket);
						...
						console.log("這個人已經在資料庫裡面，但是不再線上，不需要給一組新的guid，guid是："+result[0].user_guid);
						key = result[0].user_guid;
					...
					db.query(sql_p_sum, function(err, result) {
						...
						if(sum ==0){
							...
							console.log('電視連線：'+all_socket[ player_i ]);
							all_socket[ player_i ].emit('tv_n_player', p_name);	//第一個玩家
						}
							...
							console.log(sql_iu);
								...
								socket.emit('client_change',p_name, key, player_i, g_data, sum);
								all_socket[ player_i ].emit('tv_newplayer', p_name, key);
								//all_socket[ player_i ].emit('tv_n_player', p_name);
								socket.broadcast.emit('tv_change',player_i, player_g, g_name);
						}
...
//這個人沒有出現在inning_user也沒有出現在user  讓他新增帳號並且連線
//沒有出現在inning_user有出現在user  讓他連線
//有出現在inning_user 有出現在user  離線
					...
					var d = "DELETE from Inning_user where user_account ='"+ p_name +"'";
						console.log(d);
						db.query(d, function(err, result) {
						if(err){
							console.log(err);
				...
				var user_d = "SELECT * from User where account ='"+ p_name +"'";

				db.query(user_d, function(err, result) {
					if(err){
						console.log(err);
						return ;
					}

					console.log("這個人已經在資料庫裡面，而且在線上，不需要給一組新的guid，guid是：" + result[0].user_guid);
					...
					//console.log("這個人已經在資料庫裡面了，或是他怪怪的");


						//console.log("這個人已經在線上了");
  ...
  return "send animation!!!!!!!";
	...
	//在交換玩家之前確定這個人是不是已經結束遊戲了
		...
		//所有遊戲名稱
			...
			if(end.game == true){
				var user_data = Game_m.choose_p();//換一個玩家
...
socket.on('tile_click',function(t_id, id, t_text, c_infro){
	...
	//socket.emit('new_g', key, name, guid);
	...
	socket.on('b_click',function(id, c_infro){
		...
		var new_board = Game.choose_p(id, c_infro);


		var win = Game.winner(new_board.board, c_infro);
		//去資料庫把這人的分數改掉
